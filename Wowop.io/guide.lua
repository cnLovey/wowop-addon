...
PopulateSpells = function(dungeonId, selectedMobId)
    ...
    for _, spell_data in ipairs(sorted_spells) do
        ...
        -- Get spell description from game
        descText:SetText("|cFFFFFFFF(Loading spell description...)|r")
        
        -- Try to get spell description
        addon.TryGetSpellDescription(spellId, function(spellDesc)
            if spellDesc and spellDesc ~= "" then
                descText:SetText("|cFFFFFFFF" .. spellDesc .. "|r")
            else
                descText:SetText("|cFFFFFFFF(No description available)|r")
            end
        end)
        ...
        -- Calculate base height from description and tags
        local height = 40 -- Header height
        height = height + descText:GetStringHeight() + 4 -- Description height + padding
        height = height + (tags[1] and (tagsText:GetStringHeight() + 4) or 0) -- Tags height if present
        ...
        frame:Show()
        yOffset = yOffset + height + 8 -- Use the dynamic height for positioning the next frame
    end
    
    spellScroll:SetSize(360, yOffset) -- Set width and total content height
